#!/usr/bin/env python3
"""
Enhanced RSI Stock Tracker with Portfolio Import
A desktop application for tracking RSI and comprehensive financial metrics with portfolio import capabilities.
...
class RSIStockTracker:
    """Main application class for Enhanced RSI Stock Tracker with Portfolio Import."""
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(main_frame, 
                              text="📈 Enhanced RSI Stock Tracker with Portfolio Import", 
                              font=('Arial', 16, 'bold'),
                              bg='#1e1e1e', fg='#00ff88')
        ...
        # Bulk add section
        bulk_frame = tk.Frame(control_frame, bg='#1e1e1e')
        bulk_frame.pack(side=tk.LEFT, padx=(20, 0))
        
        tk.Button(bulk_frame, text="📁 Import Portfolio", command=self.import_portfolio,
                 bg='#8844ff', fg='white', font=('Arial', 9, 'bold')).pack(side=tk.LEFT)
        
        tk.Button(bulk_frame, text="📋 Bulk Add", command=self.bulk_add_stocks,
    ...
    def bulk_add_stocks(self):
        ...
        def add_symbols():
            ...
            self.status_label.config(text=f"Bulk added {added_count} stocks")
    ...
    def run(self):
        ...
        if not self.watchlist:
            self.status_label.config(text="Add stock symbols or import your portfolio to start tracking")
        
        # Start the GUI
        self.root.mainloop()

def main():
    """Main function to run the Enhanced RSI Stock Tracker with Portfolio Import."""
    try:
        app = RSIStockTracker()
        app.run()
    except Exception as e:
