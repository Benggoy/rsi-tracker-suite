#!/usr/bin/env python3
"""
Enhanced Working RSI Stock Tracker - Mac App Bundle Compatible Version
Fixed matplotlib backend issues for Mac app bundles
"""
...
# Fix matplotlib backend for Mac app bundles
...
try:
    ...
    # Configure matplotlib for app bundle
    matplotlib.rcParams['backend'] = 'TkAgg'
    matplotlib.rcParams['interactive'] = False
    
    # Set dark style if available
    try:
        plt.style.use('dark_background')
    except:
        pass  # Use default if dark_background not available
    
    CHARTS_AVAILABLE = True
    print("✅ Charts available - matplotlib configured for Mac app")
    
except ImportError as e:
    print(f"⚠️ Charts disabled - matplotlib import failed: {e}")
    CHARTS_AVAILABLE = False
except Exception as e:
    print(f"⚠️ Charts disabled - matplotlib configuration failed: {e}")
    CHARTS_AVAILABLE = False
...
class EnhancedRSITracker:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Enhanced RSI Tracker - Mac App Compatible")
    ...
    def setup_ui(self):
        ...
        if CHARTS_AVAILABLE:
            chart_status = "✅ Charts Working (Mac App Compatible)"
        else:
            chart_status = "⚠️ Charts Disabled (Install matplotlib)"
    ...
    def setup_tracker_tab(self):
        ...
        if CHARTS_AVAILABLE:
            ...
            tk.Button(button_frame, text="📊 Charts Disabled", command=self.show_chart_info,
                     bg='#888888', fg='white', font=('Arial', 9)).pack(side=tk.LEFT, padx=5)
        ...
        if CHARTS_AVAILABLE:
            chart_info = "Select stock + View Chart for Price/RSI/Volume analysis"
        else:
            chart_info = "Charts disabled - run 'pip3 install matplotlib' to enable"
    ...
    def setup_charts_tab(self):
        ...
        tk.Button(control_frame, text="📊 Load Chart", command=self.load_enhanced_chart,
        ...
        tk.Button(control_frame, text="🧪 Test Charts", command=self.test_charts,
                 bg='#44aaff', fg='white', font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        ...
        self.chart_message = tk.Label(self.chart_frame, 
                                     text="Charts are working in Mac App!\nSelect stock and period, then click Load Chart",
                                     bg='#1e1e1e', fg='#cccccc', font=('Arial', 12))
        self.chart_message.pack(expand=True)
    
    def setup_no_charts_tab(self):
        """Setup tab when charts are disabled"""
        charts_frame = tk.Frame(self.notebook, bg='#1e1e1e')
        self.notebook.add(charts_frame, text="⚠️ Charts Disabled")
        
        info_frame = tk.Frame(charts_frame, bg='#1e1e1e')
        info_frame.pack(expand=True, fill=tk.BOTH)
        
        tk.Label(info_frame, text="📊 Charts Not Available", 
                bg='#1e1e1e', fg='#ff8844', font=('Arial', 16, 'bold')).pack(pady=20)
        
        tk.Label(info_frame, text="To enable charts in this Mac app:", 
                bg='#1e1e1e', fg='white', font=('Arial', 12)).pack(pady=10)
        
        tk.Label(info_frame, text="1. Open Terminal", 
                bg='#1e1e1e', fg='#cccccc', font=('Arial', 11)).pack()
        
        tk.Label(info_frame, text="2. Run: pip3 install matplotlib", 
                bg='#1e1e1e', fg='#cccccc', font=('Arial', 11)).pack()
        
        tk.Label(info_frame, text="3. Restart this app", 
                bg='#1e1e1e', fg='#cccccc', font=('Arial', 11)).pack()
        
        tk.Button(info_frame, text="🔄 Test Chart Installation", command=self.test_matplotlib,
                 bg='#4488ff', fg='white', font=('Arial', 12, 'bold')).pack(pady=20)
    ...
    def test_matplotlib(self):
        ...
        try:
            ...
            matplotlib.use('TkAgg', force=True)
            import matplotlib.pyplot as plt
            messagebox.showinfo("Chart Test", "✅ Matplotlib is installed!\n\nRestart the app to enable charts.")
        except ImportError:
            messagebox.showerror("Chart Test", "❌ Matplotlib not found.\n\nRun: pip3 install matplotlib")
    
    def test_charts(self):
        """Test chart functionality"""
        if not CHARTS_AVAILABLE:
            messagebox.showwarning("Charts Disabled", "Matplotlib is not available")
            return
        ...
        try:
            ...
            ax.plot(x, y, color='#00ff88', linewidth=2, label='Test Chart')
            ax.set_title('Chart Test - Mac App Compatible', color='white', fontsize=14)
            ...
            self.status_label.config(text="✅ Chart test successful - Charts are working in Mac App!")
            messagebox.showinfo("Chart Test", "✅ Charts are working perfectly in the Mac App!")
            
        except Exception as e:
            messagebox.showerror("Chart Test Failed", f"Chart test failed: {str(e)}")
            self.status_label.config(text=f"❌ Chart test failed: {str(e)[:50]}")
    
    def show_chart_info(self):
        """Show info about disabled charts"""
        messagebox.showinfo("Charts Disabled", 
                           "Charts are disabled because matplotlib is not available.\n\n"
                           "To enable charts:\n"
                           "1. Open Terminal\n"
    ...
    def load_enhanced_chart(self):
        """Load enhanced chart with proper Mac app compatibility"""
        if not CHARTS_AVAILABLE:
            messagebox.showwarning("Charts Not Available", "Charts are disabled. Install matplotlib to enable.")
            return
        ...
        loading_label = tk.Label(self.chart_frame, 
                               text=f"Loading {symbol} chart with volume for {period}...",
                               bg='#1e1e1e', fg='#cccccc', font=('Arial', 12))
        ...
        try:
            ...
            # Create figure with Mac app compatible settings
            fig = Figure(figsize=(14, 10), facecolor='#1e1e1e', dpi=100)
            ...
            ax1.set_title(f'{company_name} ({symbol}) - Market Cap: {formatted_market_cap}', 
            ...
            ax2.set_title(f'RSI (Current: {current_rsi:.1f})', color='white', fontsize=11)
            ...
            # Create canvas with Mac app compatible settings
            self.current_canvas = FigureCanvasTkAgg(fig, self.chart_frame)
            self.current_canvas.draw()
            self.current_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            ...
            self.status_label.config(text=f"✅ Chart loaded: {symbol} ({period}) - RSI: {current_rsi:.1f} ({rsi_status}) | Volume: {self.stock_data.format_volume(current_volume)} ({volume_vs_avg})")
            
            print(f"✅ Chart loaded successfully: {symbol} ({period})")
            ...
            self.status_label.config(text=f"❌ Chart error: {error_msg}")
    ...
    def refresh_all_enhanced_stocks(self):
        ...
        self.root.after(0, lambda: self.status_label.config(text="All data updated"))
    ...
    def run(self):
        ...
        if not self.watchlist:
            if CHARTS_AVAILABLE:
                self.status_label.config(text="Add stocks to start tracking with working Mac app charts!")
            else:
                self.status_label.config(text="Add stocks to start tracking (charts disabled)")
        
        print("🚀 Starting Enhanced RSI Tracker (Mac App Compatible)...")
        if CHARTS_AVAILABLE:
            print("✅ Charts configured for Mac app compatibility!")
        else:
            print("⚠️ Charts disabled - install matplotlib: pip3 install matplotlib")
